...
// Helper: turn relative paths into absolute backend URLs (idempotent)
...
export const AudioProvider = ({ children }) => {
  ...
  const toggleFavorite = useCallback(async () => {
    ...
    if (!cur) return;
    try {
      const res = await api.patch(`/audios/${cur._id}/favorite`);
      const newFav = res.data.isFavorite;
      setCurrentAudioState(prev => prev ? { ...prev, isFavorite: newFav } : null);
      setQueue(prev => prev.map(a => a._id === cur._id ? { ...a, isFavorite: newFav } : a));
    } catch (err) {
      console.error('[AudioContext] Failed to toggle favorite:', err);
    }
  }, []);
